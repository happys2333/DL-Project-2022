...
def get_data(flag='train', dataset='ETTh1'):
    process = Process_Dataset(dataset=dataset, seq_len=SEQ_LEN,
                              label_len=LABEL_LEN, pred_len=PRED_LEN, features=FEATURES, target='OT', cols=None,
                              freq='h',
...
def train(model):
    ...
    print("loss function is %s" % loss_func.__class__.__name__)
    print("loss function is %s" % loss_func.__class__.__name__, file=log_file)
    ...
    for epoch in range(EPOCH):
        ...
        if vali_loss < min_val_loss:
            ...
            wait += 1
            print("Early stopping patience %d/%d" % (wait, PATIENCE))
            if wait == PATIENCE:
